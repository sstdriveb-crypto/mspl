...
export default function App() {
  ...
  // Cloud sync: load authoritative records from Supabase on startup
  useEffect(() => {
    const syncCloudCollections = async () => {
      try {
        const [employeesRes, attendanceRes, payslipsRes, queriesRes] = await Promise.all([
          supabase.from('employees').select('*'),
          supabase.from('attendance_logs').select('*'),
          supabase.from('payslips').select('*'),
          supabase.from('employee_queries').select('*')
        ]);

        if (employeesRes.error || attendanceRes.error || payslipsRes.error || queriesRes.error) {
          console.error('Cloud sync failed', employeesRes.error || attendanceRes.error || payslipsRes.error || queriesRes.error);
          return;
        }

        if (Array.isArray(employeesRes.data) && employeesRes.data.length > 0) {
          setEmployees(employeesRes.data as Employee[]);
        }
        if (Array.isArray(attendanceRes.data) && attendanceRes.data.length > 0) {
          setAttendanceLogs(attendanceRes.data as AttendanceLog[]);
        }
        if (Array.isArray(payslipsRes.data) && payslipsRes.data.length > 0) {
          setPayslips(payslipsRes.data as Payslip[]);
        }
        if (Array.isArray(queriesRes.data) && queriesRes.data.length > 0) {
          setEmployeeQueries(queriesRes.data as EmployeeHelpQuery[]);
        }
      } catch (err) {
  ...
  const handleEmployeeClockIn = async (selfieUrl: string, lat: number, lng: number, customDate?: string, customTime?: string) => {
    ...
    if (error) {
      console.error('Cloud attendance save failed', error);
      triggerToast('Clock-in saved locally; cloud sync failed.', 'warning');
    }
  };
  ...
  const handleUpdateEmployee = async (updated: Employee) => {
    ...
    try {
      const { error } = await supabase.from('employees').upsert([updated], { onConflict: 'id' });
      if (error) {
        console.error('Failed to sync employee update to cloud', error);
        triggerToast('Employee update saved locally; cloud sync failed.', 'warning');
      }
    } catch (err) {
      console.error('Unexpected cloud sync error for employee update', err);
      triggerToast('Employee update saved locally; cloud sync failed.', 'warning');
    }
  };
  ...
  const handleSupportSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('employee_queries').insert([newQuery]);
    if (error) {
      console.error('Cloud query dispatch failed', error);
      triggerToast('Query submitted locally; cloud sync failed.', 'warning');
    } else {
